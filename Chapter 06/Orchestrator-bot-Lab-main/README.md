...
- Configure the LUIS applications (Reservation and Weather) required for this sample.
...
- Configure Orchestrator to route utterances to LUIS/QnA language services set up above
    ...
    ```
    The *hierarchical* flag creates top level intents in the snapshot file derived from the .lu/.qna file names in the input folder.   As a result,  the example utterances are mapped to *Reservation*, *QnAMaker* and *Weather* intents/labels.
